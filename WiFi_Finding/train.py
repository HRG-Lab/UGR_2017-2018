...
room206 = pd.read_csv(open('/home/jfreking/pythonProject/wifi_216.csv'), delimiter=',',header=None)
...
plt.scatter(X[0:len(room219)-1,0],X[0:len(room219)-1,1],color='b',linewidths=5)
plt.scatter(X[len(room219):len(room219)+len(room206[:,0]),0],X[len(room219):len(room219)+len(room206[:,0]),1],color='r',linewidths=5)
plt.scatter(KMeans.cluster_centers_[:,0],KMeans.cluster_centers_[:,1], color='k',linewidths=20)
plt.show()
